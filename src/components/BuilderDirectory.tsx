...
import { Search, MapPin, Star, Phone, ExternalLink, ShieldCheck, Loader2, Zap, MessageSquare } from 'lucide-react';
import { motion } from 'motion/react';
import { db, collection, onSnapshot, query, where, OperationType, handleFirestoreError, addDoc, auth, serverTimestamp } from '../firebase';
...
export default function BuilderDirectory({ onSignup, onSelectBuilder, user }: BuilderDirectoryProps) {
  ...
  useEffect(() => {
    const path = 'builders';
    // Only show active builders (approved and paid) to the public
    // Also show builders without a status field for backward compatibility with seeded data
    const q = query(collection(db, path));
    
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const buildersData = snapshot.docs
        .map(doc => ({
          id: doc.id,
          ...doc.data()
        }))
        .filter(b => {
          // Show builders that are active OR don't have a status field (legacy/seeded data)
  ...
  const seedMockData = async () => {
    ...
    const mockData = [
      {
        name: 'Lone Star Barndos',
        location: 'Austin, TX',
        address: '456 Ranch Road, Austin, TX 78701',
        website: 'https://lonestarbarndos.example.com',
        description: 'Texas premier barndominium builder specializing in custom steel-frame homes with rustic finishes.',
        rating: 4.9,
        reviews: 124,
        specialties: ['Custom Design', 'Steel Frame', 'Turnkey'],
        phone: '(512) 555-0123',
        isVerified: true,
      ...
      {
        name: 'Mountain View Metal Homes',
        location: 'Denver, CO',
        address: '789 Peak Blvd, Denver, CO 80201',
        website: 'https://mountainviewmetal.example.com',
        description: 'Eco-friendly metal homes designed to withstand mountain climates while providing modern luxury.',
        rating: 4.7,
        reviews: 89,
        specialties: ['Eco-Friendly', 'Post-Frame', 'Finishing'],
        phone: '(303) 555-0199',
        isVerified: true,
      ...
      {
        name: 'Southern Steel Structures',
        location: 'Nashville, TN',
        address: '101 Music Row, Nashville, TN 37201',
        website: 'https://southernsteel.example.com',
        description: 'Specializing in large-scale residential and agricultural metal buildings across the Southeast.',
        rating: 4.8,
        reviews: 56,
        specialties: ['Large Scale', 'Agriculture', 'Residential'],
        phone: '(615) 555-0145',
        isVerified: false,
        image: 'https://images.unsplash.com/photo-1510798831971-661eb04b3739?auto=format&fit=crop&w=800&q=80',
        ownerUid: auth.currentUser?.uid,
      ...
      {
        name: 'Heartland Barndominiums',
        location: 'Des Moines, IA',
        address: '202 Prairie Way, Des Moines, IA 50301',
        website: 'https://heartlandbarndos.example.com',
        description: 'Modern rustic designs with a focus on interior finishing and loft living spaces.',
        rating: 4.6,
        reviews: 34,
        specialties: ['Interior Finishing', 'Modern Rustic', 'Loft Design'],
        phone: '(515) 555-0122',
        isVerified: true,
      ...
      {
        name: 'Pacific Steel & Stone',
        location: 'Bend, OR',
        address: '303 High Desert Dr, Bend, OR 97701',
        website: 'https://pacificsteelstone.example.com',
        description: 'Luxury barndominiums built for the high desert, featuring solar-ready roofing and stone accents.',
        rating: 4.9,
        reviews: 72,
        specialties: ['High Desert Build', 'Luxury Barndos', 'Solar Ready'],
        phone: '(541) 555-0188',
        isVerified: true,
    ...
    try {
      for (const builder of mockData) {
        await addDoc(collection(db, path), builder);
      }
        ...
        <div className="flex gap-3">
          {isAdmin && (
            <button 
              ...
              disabled={isSeeding}
              className="px-6 py-3 bg-brand-accent/10 text-brand-accent rounded-full font-bold hover:bg-brand-accent/20 transition-all flex items-center gap-2 border border-brand-accent/20"
            >
              {isSeeding ? <Loader2 className="animate-spin" size={18} /> : <Zap size={18} />}
              Seed Mock Data
            </button>
          )}
